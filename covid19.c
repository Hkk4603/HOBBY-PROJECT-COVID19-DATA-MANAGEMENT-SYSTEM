...
void display_doc_menu(char doc_id[])
{
    system("cls");
    ...
    while (!feof(fp))
    {
        fgets(doc_id_comp, sizeof(doc_id_comp), fp);
        if ((strcmp(doc_id, doc_id_comp) == 0) && (count == count1))
        {
            flag = 1;
            goto label_scan;
        }
...
label_scan:
    ...
    fscanf(fp, "%s", doc_name);
    // fscanf(fp, " %s", hosp_name);

    while (1)
    {
        printf("%s\n", hosp_name);
        printf("WELCOME %s \nID: %s", doc_name, doc_id);
    label_menu:
...
label_patient:
    ...
    printf("Vaccination Status Will Be Displayed After Updating\n");
    printf("\nPlese press ENTER to continue\n\n");
    getch();
    redirecting("to menu");
    return;
}
...
{
    char aadh_cmp[13], vac_name[10];
    int i, check = 0, dose, ch;
    char covi[10] = "COVISHIELD"; 
    char covax[10] = "COVAXIN";
    ...
    ch_entry:
        ...
        {
        case 1:
            strcpy(vac_name, covax);
            break;
        case 2:
            strcpy(vac_name, covi);
            break;
        default:
...
void patient_menue(char *ph_no, char *patient_aadhar, char *dummy_name)
{
    ...
    printf("1.Book Vaccination Slot\n\n2.Check Vaccination Status\n\n3.Logout\n\n");
    printf("Enter Your Choice: ");
    scanf("%d", &ch);
    switch (ch)
    {
